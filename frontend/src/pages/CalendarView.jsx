...
export default function CalendarView() {
  ...
  return (
    <div className="h-full flex flex-col space-y-6 max-w-7xl mx-auto pb-4">
      ...
      <div className={`rounded-3xl border backdrop-blur-3xl shadow-2xl overflow-hidden flex-1 flex flex-col relative z-10 ${isDarkMode ? 'bg-[#1E293B]/80 border-white/10' : 'bg-white/95 border-slate-200'}`}>
         ...
         {/* Command Bar */}
         <div className={`p-6 border-b flex flex-col sm:flex-row justify-between items-center backdrop-blur-md gap-4 ${isDarkMode ? 'border-white/10 bg-[#1E293B]/40' : 'border-slate-200 bg-slate-50/80'}`}>
           <div className="flex items-center gap-6">
             ...
             <div className={`hidden md:flex items-center rounded-xl border overflow-hidden shadow-inner flex-shrink-0 ${isDarkMode ? 'bg-[#1E293B] border-white/10' : 'bg-white border-slate-300'}`}>
               <button onClick={handlePrevMonth} className={`p-2.5 border-r transition-colors text-sm ${isDarkMode ? 'text-slate-300 hover:bg-[#1E293B]/5 hover:text-[#F97316] border-white/10' : 'text-slate-600 hover:bg-slate-100 hover:text-indigo-600 border-slate-300'}`}><ChevronLeft className="w-5 h-5" /></button>
               <button onClick={handleToday} className={`px-5 py-2.5 text-[10px] font-medium transition-colors ${isDarkMode ? 'text-slate-200 hover:bg-[#1E293B]/5 hover:text-[#F97316]' : 'text-slate-700 hover:bg-slate-100 hover:text-indigo-600'}`}>Jump to Today</button>
               <button onClick={handleNextMonth} className={`p-2.5 border-l transition-colors text-sm ${isDarkMode ? 'text-slate-300 hover:bg-[#1E293B]/5 hover:text-[#F97316] border-white/10' : 'text-slate-600 hover:bg-slate-100 hover:text-indigo-600 border-slate-300'}`}><ChevronRight className="w-5 h-5" /></button>
             </div>
           </div>
              ...
              {calendarGrids.map((displayDate, index) => {
                ...
                return (
                  <div key={index} className={`min-h-[120px] p-2.5 rounded-2xl border transition-all duration-300 relative group overflow-hidden ${
                      isCurrentMonth 
                      ? (isDarkMode ? 'bg-[#1E293B]/40 border-white/5 hover:border-[#F97316]/30 hover:bg-[#1E293B]/80 shadow-sm' : 'bg-slate-50 border-slate-200 hover:border-slate-300 hover:bg-slate-100 hover:shadow-md') 
