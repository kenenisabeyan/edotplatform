    ...
    <div className="flex justify-between items-start mb-4">
      ...
      {percentage && (
        <span className={`text-[10px]  font-black  px-2 py-0.5 rounded-md border ${
          type === 'primary' ? 'bg-[#11151F]/20 border-white/30 text-white' : 
...
export default function FinanceFees() {
  ...
  return (
    <div className="space-y-6">
      <div className={`flex justify-between items-center mb-8 backdrop-blur-xl p-6 rounded-3xl border shadow-2xl ${isDarkMode ? 'bg-[#0B0E14]/90 border-white/10' : 'bg-white/95 border-slate-200'}`}>
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`lg:col-span-2 backdrop-blur-xl p-8 rounded-3xl border shadow-2xl ${isDarkMode ? 'bg-[#0B0E14]/90 border-white/10' : 'bg-white/95 border-slate-200'}`}>
      ...
      <div className={`backdrop-blur-xl rounded-3xl border shadow-2xl overflow-hidden ${isDarkMode ? 'bg-[#0B0E14]/90 border-white/10' : 'bg-white/95 border-slate-200'}`}>
        <div className={`p-6 border-b flex flex-col md:flex-row justify-between items-center gap-4 ${isDarkMode ? 'border-white/10 bg-[#11151F]/5' : 'border-slate-200 bg-slate-50'}`}>
          ...
          <div className="flex gap-4 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
              ...
              <input 
                ...
                className={`w-full pl-11 pr-4 py-2.5 bg-[#11151F] border rounded-xl text-sm font-medium outline-none focus:border-[#FFD700]/50 focus:ring-1 focus:ring-[#FFD700]/50 placeholder:text-slate-300 transition-all shadow-inner ${isDarkMode ? 'border-white/10 text-white' : 'border-slate-200 text-slate-900'}`}
              />
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left whitespace-nowrap">
            ...
            <tbody className={`divide-y divide-white/5 text-sm font-normal ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
              {courses.length > 0 ? courses.map(c => {
                 ...
                 return (
                <tr key={c.id} className="hover:bg-[#11151F]/5 transition-colors">
