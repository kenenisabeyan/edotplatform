  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        {/* Engagement Trend */}
        <div className={`xl:col-span-2 backdrop-blur-xl p-8 rounded-3xl border shadow-2xl ${isDarkMode ? 'bg-[#0B0E14]/90 border-white/10' : 'bg-white/95 border-slate-200'}`}>
        ...
        <div className={`backdrop-blur-xl rounded-3xl border shadow-2xl overflow-hidden flex flex-col ${isDarkMode ? 'bg-[#0B0E14]/90 border-white/10' : 'bg-white/95 border-slate-200'}`}>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 flex flex-col gap-4">
             {topPerformers.length > 0 ? (
                topPerformers.map((student, idx) => (
                   <div key={idx} className={`flex justify-between items-center bg-[#11151F] border px-4 py-3 rounded-2xl hover:border-white/10 transition-colors ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
                      <div>
                      ...
                      <div className="flex flex-col items-end">
                         ...
                         <span className={`text-[9px] font-bold px-2 py-0.5 rounded border mt-1 ${isDarkMode ? 'text-slate-300 bg-[#11151F]/5 border-white/5' : 'text-slate-500 bg-slate-50 border-slate-100'}`}>{student.grade}</span>
                      </div>
                   </div>
                ))
             ) : (
                <div className={`flex flex-col items-center justify-center p-4 border-2 border-dashed rounded-2xl w-full h-full shadow-inner ${isDarkMode ? 'border-white/10 bg-[#11151F]/5' : 'border-slate-200 bg-slate-50'}`}>
