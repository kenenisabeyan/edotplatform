...
export default function LibraryView() {
  ...
  if (isBlocked) {
    return (
      <div className={`min-h-screen flex items-center justify-center backdrop-blur-2xl p-4 ${isDarkMode ? 'bg-[#0B0E14]/90' : 'bg-white/95'}`}>
        <div className={`max-w-lg w-full border border-[#FFD700] backdrop-blur-xl p-8 rounded-3xl shadow-2xl ${isDarkMode ? 'bg-[#11151F]/10' : 'bg-slate-50'}`}>
  ...
  return (
    ...
    >
      ...
      {user?.role === 'admin' && (
        <div className={`rounded-3xl border p-4 mt-4 shadow-xl ${isDarkMode ? 'border-white/10 bg-[#0B0E14]/90' : 'border-slate-200 bg-white/95'}`}>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className={`rounded-2xl border p-3 ${isDarkMode ? 'border-white/10 bg-[#0B0E14]' : 'border-slate-200 bg-white'}`}>
              ...
              {enrollmentRequests.length === 0 ? (
                ...
                <div key={req.id} className={`mb-2 p-2 rounded-lg ${isDarkMode ? 'bg-[#11151F]/5' : 'bg-slate-50'}`}>
            ...
            <div className={`rounded-2xl border p-3 ${isDarkMode ? 'border-white/10 bg-[#0B0E14]' : 'border-slate-200 bg-white'}`}>
              ...
              {courses.length === 0 ? (
                ...
                <div key={course.id} className={`mb-2 p-2 rounded-lg border ${isDarkMode ? 'bg-[#11151F]/5 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    <button 
                      ...
                      className={`px-3 py-1.5 text-xs font-bold rounded-lg border hover:border-[#FFD700]/50 hover:text-[#FFD700] transition-colors ${isDarkMode ? 'bg-[#11151F]/5 border-white/10 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`}
                    >
                      Edit Curriculum
                    </button>
                  </div>
          ...
          <div className="mt-3">
            ...
            {resources.slice(0, 3).map((res) => (
              <div key={res.id} className={`mb-2 flex items-center justify-between p-2 rounded-lg ${isDarkMode ? 'bg-[#11151F]/5' : 'bg-slate-50'}`}>
      ...
      <div className={`flex items-center overflow-x-auto scrollbar-hide gap-3 border rounded-2xl p-2 w-max shadow-sm backdrop-blur-md ${isDarkMode ? 'bg-[#11151F]/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
      ...
      {/* Upload Form Engine */}
      {showUploadForm && (
        <div className={`rounded-3xl border backdrop-blur-xl p-6 shadow-sm relative overflow-hidden ${isDarkMode ? 'border-white/5 bg-[#11151F]/5' : 'border-slate-100 bg-slate-50'}`}>
          ...
          <form onSubmit={handleUploadSubmit} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-1 md:col-span-1">
                    ...
                    <input 
                      ...
                      className={`w-full border text-base font-normal rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#FFD700] placeholder-slate-500 ${isDarkMode ? 'bg-[#0B0E14] border-white/10 text-white' : 'bg-white border-slate-200 text-slate-900'}`}
                    />
                </div>
                <div className="space-y-1">
                    ...
                    <input 
                      ...
                      className={`w-full border rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#FFD700] font-semibold placeholder-slate-500 ${isDarkMode ? 'bg-[#0B0E14] border-white/10 text-white' : 'bg-white border-slate-200 text-slate-900'}`}
                    />
                </div>
                <div className="space-y-1">
            ...
            <div className="space-y-1">
                ...
                <div className={`relative border-2 border-dashed rounded-2xl p-6 bg-[#0B0E14]/50 hover:bg-[#11151F]/5 transition-colors group cursor-pointer ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                    <input 
                      type="file" 
                      required
                    ...
                    <div className="flex flex-col items-center justify-center text-center space-y-2">
                       <div className={`w-12 h-12 rounded-full text-[#E30A17] flex items-center justify-center group-hover:scale-110 transition-transform ${isDarkMode ? 'bg-[#11151F]/5' : 'bg-slate-50'}`}>
      ...
      {activeContainer === 'download' && (
        <>
          {filteredResources.length === 0 && !showUploadForm ? (
            <div className={`p-12 text-center rounded-3xl border backdrop-blur-xl shadow-sm flex flex-col items-center justify-center ${isDarkMode ? 'border-white/5 bg-[#11151F]/5' : 'border-slate-100 bg-slate-50'}`}>
              <div className={`w-20 h-20 border rounded-full flex items-center justify-center mb-4 ${isDarkMode ? 'bg-[#11151F]/5 text-slate-200 border-white/10' : 'bg-slate-50 text-slate-600 border-slate-200'}`}>
              ...
              {filteredResources.map((resource) => {
                ...
                return (
                  <motion.div 
                    ...
                    key={resource.id} 
                    className={`rounded-3xl border backdrop-blur-xl shadow-2xl overflow-hidden flex flex-col group hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] transition-all relative ${isDarkMode ? 'border-white/10 bg-[#0B0E14]/90' : 'border-slate-200 bg-white/95'}`}
                  >
                    {isOwner && (
                      <button
                        onClick={() => handleDelete(resource.id)}
                        title="Delete Resource"
                        className={`absolute top-3 right-3 hover:bg-[#E30A17]/20 text-[#E30A17] border border-transparent hover:border-[#E30A17]/30 p-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all z-10 shadow-sm backdrop-blur ${isDarkMode ? 'bg-[#11151F]/10' : 'bg-slate-50'}`}
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                    )}

                    <div className={`h-40 flex items-center justify-center relative overflow-hidden group-hover:bg-[#FFD700]/5 transition-colors ${isDarkMode ? 'bg-[#0B0E14]' : 'bg-white'}`}>
      ...
      {activeContainer === 'secure' && (
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-4">
          <div className={`rounded-3xl border p-4 ${isDarkMode ? 'border-white/10 bg-[#11151F]/5' : 'border-slate-200 bg-slate-50'}`}>
              ...
              <div className="space-y-2">
                {filteredResources.map((resource) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-lg ${secureResource?.id === resource.id ? 'bg-cyan-500 ' : 'bg-[#11151F]/10 text-slate-100 hover:bg-[#11151F]/20'} ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                    {resource.title}
                  </button>
                ))}
              </div>
          ...
          <div className={`rounded-3xl border p-4 ${isDarkMode ? 'border-white/10 bg-[#11151F]/5' : 'border-slate-200 bg-slate-50'}`} onContextMenu={(e) => user?.role === 'student' && e.preventDefault()}>
      ...
      {activeContainer === 'wiki' && (
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          <aside className={`w-full lg:w-72 shrink-0 rounded-3xl border overflow-hidden shadow-2xl sticky top-24 ${isDarkMode ? 'border-white/10 bg-[#0B0E14]' : 'border-slate-200 bg-white'}`}>
            <div className={`p-6 border-b ${isDarkMode ? 'border-white/5 bg-[#13161B]/50' : 'border-slate-100 bg-slate-50'}`}>
            ...
            <div className="p-4">
              ...
              <ul className="space-y-1 text-sm font-medium">
                <li>
                  <a href="#overview" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#11151F]/60 hover:text-white transition-all group ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                ...
                <li>
                  <a href="#workflows" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#11151F]/60 hover:text-white transition-all group ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                ...
                <li>
                  <a href="#roles" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#11151F]/60 hover:text-white transition-all group ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                ...
                <li>
                  <a href="#submission" className={`flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#11151F]/60 hover:text-white transition-all group ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
          ...
          <section className={`flex-1 rounded-3xl border shadow-2xl overflow-hidden ${isDarkMode ? 'border-white/10 bg-[#0B0E14]' : 'border-slate-200 bg-white'}`}>
            <div className="p-8 lg:p-14 max-w-4xl mx-auto space-y-16">
              ...
              <div className={`border-b pb-8 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                <div className={`inline-flex items-center gap-1.5 px-3 py-1 mb-4 rounded-full border text-xs font-semibold ${isDarkMode ? 'bg-[#11151F]/50 border-white/10 text-slate-300' : 'bg-slate-100 border-slate-200 text-slate-500'}`}>
              ...
              <article id="workflows" className="scroll-mt-24">
                ...
                <div className={`p-6 rounded-2xl border leading-relaxed mb-6 ${isDarkMode ? 'bg-[#11151F]/40 border-white/5 text-slate-300' : 'bg-slate-50 border-slate-100 text-slate-500'}`}>
                  <div className="flex flex-col md:flex-row items-center gap-4 text-sm font-semibold">
                    <div className={`px-4 py-2 bg-slate-800 rounded-lg border border-slate-600 w-full md:w-auto text-center ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Instructor Upload</div>
      ...
      {user?.role === 'admin' && (
        <div className={`rounded-3xl border p-4 mt-4 ${isDarkMode ? 'border-white/10 bg-[#11151F]/5' : 'border-slate-200 bg-slate-50'}`}>
          ...
          {pendingQueue.length === 0 ? (
            ...
            <div key={item.id} className={`mb-2 p-3 border rounded-xl ${isDarkMode ? 'border-white/10 bg-[#0B0E14]/80' : 'border-slate-200 bg-white/90'}`}>
      ...
      {reviewModalOpen && reviewTarget && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className={`w-full max-w-xl border rounded-3xl p-6 ${isDarkMode ? 'bg-[#0B0E14] border-white/10' : 'bg-white border-slate-200'}`}>
            ...
            <textarea
              ...
              className={`w-full h-28 p-3 rounded-xl border outline-none mb-3 ${isDarkMode ? 'bg-[#0B0E14] border-white/10 text-white' : 'bg-white border-slate-200 text-slate-900'}`}
              placeholder="Leave feedback or request corrections..."
            />
            <div className="flex gap-2 mb-4">
              <button onClick={() => setReviewDecision('approved')} className={`px-3 py-2 rounded-lg text-sm ${reviewDecision === 'approved' ? 'bg-[#E67E22] ' : 'bg-[#11151F]/10 '} ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Approve</button>
              <button onClick={() => setReviewDecision('rejected')} className={`px-3 py-2 rounded-lg text-sm ${reviewDecision === 'rejected' ? 'bg-[#E30A17] ' : 'bg-[#11151F]/10 '} ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Reject</button>
            </div>
            <div className="flex justify-end gap-2">
              <button onClick={() => setReviewModalOpen(false)} className={`px-4 py-2 rounded-lg ${isDarkMode ? 'bg-[#11151F]/10 text-white' : 'bg-slate-50 text-slate-900'}`}>Cancel</button>
