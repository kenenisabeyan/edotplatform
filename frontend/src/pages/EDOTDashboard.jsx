...
export default function EDOTDashboard() {
  ...
  const SmartCard = ({ title, value, icon: Icon }) => {
    ...
    return (
      <motion.div whileHover={{ y: -5 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }}>
        <Card hover={false} className={`rounded-2xl p-6 border backdrop-blur-xl flex flex-col justify-between group relative overflow-hidden transition-all duration-300 ${glowClass} ${isDarkMode ? 'bg-[#11151F]/5 border-white/5' : 'bg-white border-slate-200 shadow-sm'}`}>
          <div className="flex items-center gap-4 mb-6">
            <div className={`w-10 h-10 rounded-xl border flex items-center justify-center ${isDarkMode ? 'bg-[#11151F]/10 border-white/20 text-white' : 'bg-slate-100 border-slate-200 text-slate-800'}`}>
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className={`backdrop-blur-xl p-4 rounded-xl border relative shadow-2xl ${isDarkMode ? 'bg-[#0B0E14]/95 border-white/10' : 'bg-white/95 border-slate-200'}`}>
  ...
  return (
    ...
    >
      ...
      <div className={`rounded-2xl p-8 border relative overflow-hidden backdrop-blur-xl ${isDarkMode ? 'bg-[#11151F]/5 border-white/5' : 'bg-white border-slate-200 shadow-sm'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <Card hover={false} className={`lg:col-span-3 rounded-2xl p-6 border backdrop-blur-xl shadow-lg flex flex-col items-center justify-center relative min-h-[350px] ${isDarkMode ? 'bg-[#11151F]/5 border-white/5' : 'bg-white border-slate-200'}`}>
        ...
        <Card hover={false} className={`lg:col-span-6 rounded-2xl p-6 border backdrop-blur-xl shadow-lg flex flex-col min-h-[350px] ${isDarkMode ? 'bg-[#11151F]/5 border-white/5' : 'bg-white border-slate-200'}`}>
          <div className="flex justify-between items-center mb-6 shrink-0">
            <h3 className={`font-semibold text-sm ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{areaConfig.title}</h3>
            <div className={`flex items-center gap-1 text-[10px] px-2 py-1 rounded ${isDarkMode ? 'text-slate-200 bg-[#11151F]/5' : 'text-slate-600 bg-slate-100'}`}>
         ...
         <div className="lg:col-span-3 h-full">
           ...
           {widgetConfig.type === 'claim' && (
             <Card hover={false} className={`rounded-2xl p-6 border backdrop-blur-xl shadow-lg flex flex-col min-h-[350px] ${isDarkMode ? 'bg-[#11151F]/5 border-white/5' : 'bg-white border-slate-200'}`}>
