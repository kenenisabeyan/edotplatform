                        ...
                        <div className={`p-6 rounded-3xl border shadow-sm flex flex-col justify-between ${isDarkMode ? 'border-white/5 bg-[#111827]' : 'border-slate-200 bg-white'}`}>
                           <div>
                              <h4 className={`text-sm font-bold uppercase tracking-wider mb-5 flex items-center gap-3 ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                                 <BadgeCheck className="w-5 h-5 text-[#F97316]" /> Managed Courses ({inst.coursesTaught?.length || 0})
                              </h4>
                              <div className="flex flex-wrap gap-2 mb-6">
                                 {inst.coursesTaught?.length > 0 ? inst.coursesTaught.map(c => (
                                    <span key={c.id} className="px-3 py-1.5 bg-[#F97316]/10 text-[#F97316] border border-[#F97316]/20 rounded-xl text-xs font-bold">
                                       {c.title}
                                    </span>
                           ...
                           <div className={`pt-5 border-t flex flex-col gap-3 ${isDarkMode ? 'border-white/10' : 'border-slate-100'}`}>
                              ...
                              <div className="flex flex-col gap-3">
                                 <select value={selectedCourseCat} onChange={e => { setSelectedCourseCat(e.target.value); setSelectedCourse(''); }} className={`w-full text-sm px-4 py-3 border rounded-2xl outline-none focus:ring-2 focus:ring-[#F97316]/50 transition-all ${isDarkMode ? 'bg-black/40 border-white/10 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`}>
                                    <option value="">1. Select Category...</option>
                                    {learnerGroups.map(g => <option key={g.id} value={g.name}>{g.name}</option>)}
                                    <option value="Uncategorized">Uncategorized</option>
                                 </select>
                                 <div className="flex gap-2 items-center">
                                    <select value={selectedCourse} onChange={e => setSelectedCourse(e.target.value)} disabled={!selectedCourseCat} className={`flex-1 min-w-0 text-sm px-4 py-3 border rounded-2xl outline-none focus:ring-2 focus:ring-[#F97316]/50 transition-all ${isDarkMode ? 'bg-black/40 border-white/10 text-white disabled:opacity-50' : 'bg-slate-50 border-slate-200 text-slate-900 disabled:bg-slate-100 disabled:text-slate-400'}`}>
                                    ...
                                    <button onClick={() => handleAssignCourse(inst.id)} disabled={!selectedCourse} className="px-5 py-3 bg-[#F97316] hover:bg-[#ea580c] disabled:opacity-50 disabled:hover:translate-y-0 text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 rounded-full text-sm font-bold transition-all">
                                       Assign
                                    </button>
                                 </div>
                              </div>
