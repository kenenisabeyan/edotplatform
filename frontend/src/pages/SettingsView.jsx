...
export default function SettingsView() {
  ...
  const ToggleSwitch = ({ label, description, checked, onChange }) => (
    <div className={`flex items-center justify-between p-5 rounded-2xl border transition-all group shadow-sm ${isDarkMode ? 'bg-[#0B1120] border-white/5 hover:border-[#00D4FF]/30' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}>
      <div className="flex items-start gap-4 pr-6">
        <div>
          <h4 className={`font-bold text-sm transition-colors ${isDarkMode ? 'text-white group-hover:text-[#00D4FF]' : 'text-slate-800 group-hover:text-indigo-600'}`}>{label}</h4>
      ...
      <button 
        ...
        className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors shrink-0 outline-none focus:ring-2 focus:ring-offset-2 ${isDarkMode ? 'focus:ring-offset-[#0B1120] focus:ring-[#00D4FF]' : 'focus:ring-offset-white focus:ring-indigo-500'} ${checked ? (isDarkMode ? 'bg-[#00D4FF]' : 'bg-indigo-600') : (isDarkMode ? 'bg-slate-700' : 'bg-slate-300')}`}
      >
    ...
    <div className={`p-5 rounded-2xl border transition-all shadow-sm ${isDarkMode ? 'bg-[#0B1120] border-white/5 hover:border-white/10' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}>
      ...
      <div className="px-1">
        <input 
          ...
          className={`w-full h-1.5 rounded-lg appearance-none cursor-pointer ${isDarkMode ? 'bg-slate-700 accent-[#00D4FF]' : 'bg-slate-200 accent-indigo-600'}`}
        />
  ...
  const InputField = ({ label, description, type = "text", value, onChange, placeholder, icon: Icon }) => (
    <div className={`p-5 rounded-2xl border transition-all shadow-sm ${isDarkMode ? 'bg-[#0B1120] border-white/5 hover:border-white/10' : 'bg-slate-50 border-slate-200 hover:border-slate-300'}`}>
      <label className={`block text-xs font-black mb-2 flex items-center gap-2 ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-700'}`}>
      ...
      <div className="relative">
        <input 
          ...
          className={`w-full !px-5 !py-3.5 border !rounded-full font-medium text-sm outline-none focus:ring-2 focus:border-transparent transition-all shadow-inner ${isDarkMode ? 'bg-[#0B1120] border-white/10 text-white focus:ring-[#00D4FF] placeholder:text-slate-300' : 'bg-white border-slate-300 text-slate-900 focus:ring-indigo-500 placeholder:text-slate-400'}`}
        />
      </div>
    </div>
  );
    ...
    <div className="animate-in fade-in flex flex-col space-y-8 min-h-screen p-6 md:p-10 max-w-none w-full">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl text-left font-bold text-sm transition-all whitespace-nowrap outline-none ${
                  activeTab === tab.id 
                  ? (isDarkMode ? 'bg-[#00D4FF]/10 text-[#00D4FF] border border-[#00D4FF]/20 shadow-inner' : 'bg-indigo-50 text-indigo-600 border border-indigo-200 shadow-inner') 
                  : (isDarkMode ? 'text-slate-200 hover:bg-white/5/5 hover:text-white border border-transparent focus:ring-2 focus:ring-[#00D4FF]/50' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900 border border-transparent focus:ring-2 focus:ring-indigo-500/50')
                }`}
              >
        ...
        <div className={`flex-1 backdrop-blur-xl border p-6 md:p-8 rounded-3xl shadow-2xl min-h-[500px] relative overflow-hidden ${isDarkMode ? 'bg-[#0B1120]/80 border-white/10' : 'bg-white/95 border-slate-200'}`}>
          ...
          <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-gradient-to-br from-[#00D4FF]/3 to-[#008A32]/3 rounded-full blur-[100px] pointer-events-none -z-10"></div>
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-300 z-10 relative space-y-8">
            ...
            {activeTab === 'general' && (
              <>
                ...
                <div className="space-y-4">
                  <label className={`block text-xs font-black ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-700'}`}>Timezone Protocol</label>
                  <CustomDropdown
            ...
            {activeTab === 'security' && (
              <>
                ...
                <div className={`p-6 rounded-2xl border flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm ${isDarkMode ? 'bg-[#0B1120] border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                  <div>
                    <h4 className={`font-bold text-sm flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-slate-800'}`}><Fingerprint className={`w-4 h-4 ${isDarkMode ? 'text-[#00D4FF]' : 'text-indigo-600'}`}/> Biometric / Two-Factor Protocol</h4>
            ...
            {activeTab === 'pedagogy' && (
              <>
                <div className={`border-b pb-4 mb-6 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                  <h3 className={`text-lg font-black ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-900'}`}>Pedagogy Logic</h3>
            ...
            {activeTab === 'ai' && (
              <>
                <div className={`border-b pb-4 mb-6 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                  <h3 className={`text-lg font-black ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-900'}`}>Automation & AI</h3>
            ...
            {activeTab === 'platform' && (
              <>
                <div className={`border-b pb-4 mb-6 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                  <h3 className={`text-lg font-black ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-900'}`}>Platform DNA</h3>
                ...
                <div className={`p-8 rounded-2xl border flex flex-col justify-center mb-6 shadow-sm ${isDarkMode ? 'bg-[#0B1120] border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                  <label className={`block text-xs font-black mb-4 flex items-center gap-2 ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-700'}`}><Palette className="w-4 h-4"/> Global Hex Code</label>
            ...
            {activeTab === 'ledger' && (
              <>
                <div className={`border-b pb-4 mb-6 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                  <h3 className={`text-lg font-black ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-900'}`}>Ledger & Intelligence</h3>
            ...
            {activeTab === 'privacy' && (
              <>
                <div className={`border-b pb-4 mb-6 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                  <h3 className={`text-lg font-black ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-900'}`}>Transparency Matrix</h3>
            ...
            {activeTab === 'family' && (
              <>
                <div className={`border-b pb-4 mb-6 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                  <h3 className={`text-lg font-black ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-900'}`}>Family Supporter Link</h3>
            ...
            {activeTab === 'billing' && (
              <>
                <div className={`border-b pb-4 mb-6 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                  <h3 className={`text-lg font-black ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-900'}`}>Subscriptions & Billing</h3>
                ...
                <div className={`p-6 rounded-2xl border shadow-sm ${isDarkMode ? 'bg-[#0B1120] border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                  <label className={`block text-xs font-black mb-4 flex items-center gap-2 ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-700'}`}><CreditCard className="w-4 h-4"/> Default Payment Protocol</label>
                  <CustomDropdown
            ...
            {activeTab === 'alerts' && (
              <>
                <div className={`border-b pb-4 mb-6 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                  <h3 className={`text-lg font-black ${isDarkMode ? 'text-[#00D4FF]' : 'text-slate-900'}`}>Support Thresholds</h3>
