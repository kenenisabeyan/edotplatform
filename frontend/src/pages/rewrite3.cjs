...
const matchStr = '  const groupableContacts = contacts.filter(contact => contact.type === \\\'user\\\' && contact.id !== user?.id);\\n  const filteredContacts = contacts.filter(c => \\n    c.name.toLowerCase().includes(searchQuery.toLowerCase())\\n  );\\n\\n  return (';
// Wait, the string in the file has actual newlines, so we must match it correctly.
// A simpler way:
const parts = content.split('\\n  return (');
// we want the LAST one that is right before the <div>
// But let's just find the index of "  const groupableContacts =" and slice until there.
...
const newReturnBlock = \`  const groupableContacts = contacts.filter(contact => contact.type === 'user' && contact.id !== user?.id);
  const filteredContacts = contacts.filter(c => 
    c.name.toLowerCase().includes(searchQuery.toLowerCase())
  );
