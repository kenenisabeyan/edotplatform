  ...
  return (
    ...
    >
      <div className={`flex flex-col sm:flex-row justify-between items-start sm:items-center rounded-3xl p-6 md:p-8 bg-gradient-to-br from-white/5 to-transparent border backdrop-blur-xl shadow-2xl relative overflow-hidden ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
        ...
        <div className={`relative z-10 bg-[#1E293B] flex rounded-xl border p-1 shadow-lg ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
           <button 
      ...
      {activeTab === 'pending' && (courses.length === 0 && pendingEnrollments.length === 0) ? (
          ...
          <div className="grid grid-cols-1 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            {courses.length > 0 && (
              <div className={`rounded-3xl p-6 border backdrop-blur-xl shadow-2xl ${isDarkMode ? 'bg-[#1E293B]/40 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
                ...
                <div className="grid gap-6">
                  ...
                  {courses.map(c => (
                    <motion.div 
                      ...
                      key={c.id} 
                      className={`rounded-2xl border shadow-inner overflow-hidden flex flex-col md:flex-row group transition-all ${isDarkMode ? 'border-white/10 bg-[#1E293B]' : 'border-slate-200 bg-white'}`}
                    >
                      ...
                      <div className={`flex flex-col flex-1 relative z-10 -mt-6 md:mt-0 md:bg-transparent rounded-t-3xl md:rounded-none ${isDarkMode ? 'bg-[#1E293B]/90' : 'bg-white/95'}`}>
                        <div className="p-6 flex-1">
                          ...
                          <div className={`flex flex-wrap gap-4 mb-4 text-[10px] font-black ${isDarkMode ? 'text-slate-200' : 'text-slate-600'}`}>
                             <span className={`flex items-center gap-1 border px-2 py-1 rounded ${isDarkMode ? 'border-white/5 bg-[#1E293B]/5' : 'border-slate-100 bg-slate-50'}`}>Instructor: <span className={isDarkMode ? 'text-white' : 'text-slate-900'}>{c.instructor?.name || 'Unknown'}</span></span>
                        ...
                        <div className={`p-5 border-t flex flex-col sm:flex-row justify-between items-center gap-5 ${isDarkMode ? 'border-white/5 bg-[#1E293B]/50' : 'border-slate-100 bg-slate-100'}`}>
            ...
            {pendingEnrollments.length > 0 && (
              <div className={`rounded-3xl p-6 border backdrop-blur-xl shadow-2xl mt-4 ${isDarkMode ? 'bg-[#1E293B]/40 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {pendingEnrollments.map(en => (
                    <motion.div 
                      ...
                      exit={{ opacity: 0, scale: 0.9 }}
                      key={en.id} 
                      className={`rounded-2xl border p-5 flex flex-col justify-between hover:border-[#F97316]/30 transition-colors group shadow-lg relative overflow-hidden ${isDarkMode ? 'border-white/10 bg-[#1E293B]' : 'border-slate-200 bg-white'}`}
                    >
                      ...
                      <div className="relative z-10">
                        <div className="flex items-start justify-between mb-2">
                           ...
                           <span className={`text-[9px] font-black border px-2 py-0.5 rounded whitespace-nowrap ${isDarkMode ? 'text-slate-300 bg-[#1E293B]/5 border-white/10' : 'text-slate-500 bg-slate-50 border-slate-200'}`}>ID: {en.id.slice(0,6)}</span>
                        </div>
          ...
          <div className={`p-16 text-center rounded-3xl border shadow-2xl backdrop-blur-xl flex flex-col items-center justify-center relative overflow-hidden animate-in fade-in zoom-in-95 duration-500 ${isDarkMode ? 'border-white/10 bg-[#1E293B]/90' : 'border-slate-200 bg-white/95'}`}>
            <div className={`w-24 h-24 border rounded-full flex items-center justify-center mb-6 relative group border border-blue-500/30 text-blue-500 ${isDarkMode ? 'bg-[#1E293B]/5 text-slate-200 border-white/10' : 'bg-slate-50 text-slate-600 border-slate-200'}`}>
