      ...
      <div className={`flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b pb-6 pt-2 mb-8 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
        ...
        <div className={`relative z-10 flex rounded-xl border p-1 shadow-lg ${isDarkMode ? 'bg-[#0B1120] border-white/10' : 'bg-slate-100 border-slate-200'}`}>
           <button 
             ...
             className={`px-6 py-2.5 rounded-lg text-xs font-black transition-all ${activeTab === 'pending' ? 'bg-[#F97316]/20 text-[#F97316] shadow-[0_0_15px_rgba(249,115,22,0.2)]' : (isDarkMode ? 'text-slate-300 hover:text-white' : 'text-slate-500 hover:text-slate-900')}`}
           >
             Pending Queue
           </button>
           <button 
                  ...
                  {courses.map(c => (
                    ...
                    >
                      <div className="w-full md:w-64 h-56 md:h-auto shrink-0 relative bg-black/40">
                        ...
                        <div className={`absolute top-4 left-4 backdrop-blur-sm px-3 py-1.5 rounded-lg text-xs font-black text-[#F97316] shadow-sm border ${isDarkMode ? 'bg-[#0B1120]/80 border-white/10' : 'bg-white/90 border-slate-200'}`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {pendingEnrollments.map(en => (
                    <motion.div 
                      ...
                      exit={{ opacity: 0, scale: 0.9 }}
                      key={en.id} 
                      className={`rounded-2xl border p-5 flex flex-col justify-between hover:border-[#F97316]/30 transition-colors group shadow-lg relative overflow-hidden ${isDarkMode ? 'border-white/10 bg-[#0B1120]' : 'border-slate-200 bg-white'}`}
                    >
