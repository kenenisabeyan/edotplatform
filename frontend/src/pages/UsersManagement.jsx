...
import { 
  ShieldCheck, Users, CheckCircle2, XCircle, Search, 
  LayoutGrid, List, KeyRound, Trash2, Eye, Network, Undo2
} from 'lucide-react';
    ...
    switch (role) {
      ...
      case 'instructor':
        return {
          ...
          avatarBorder: "border-indigo-400/40",
        };
      case 'student':
        return {
          isStaff: false,
          bgGrad: "from-cyan-500 to-blue-500",
          badgeBg: "bg-gradient-to-r from-cyan-500 to-[#00D4FF]",
          avatarBorder: "border-[#00D4FF]/40",
        };
      case 'parent':
        return {
          isStaff: false,
          bgGrad: "from-teal-500 to-cyan-500",
          badgeBg: "bg-gradient-to-r from-teal-500 to-cyan-500",
          avatarBorder: "border-teal-400/40",
        };
      default:
        return {
          isStaff: false,
          bgGrad: "from-slate-500 to-slate-600",
          badgeBg: "bg-slate-50",
    ...
    <div className="animate-in fade-in flex flex-col space-y-8 min-h-screen -mx-4 md:-mx-8 lg:-mx-12 -mt-4 md:-mt-8 p-6 md:p-8">
      ...
      <div className={`rounded-[2.5rem] p-8 border backdrop-blur-2xl shadow-2xl overflow-hidden relative transition-all duration-500 ${isDarkMode ? 'border-white/10 bg-[#0B1120]/80' : 'border-slate-200 bg-white/90 hover:border-[#2563EB]/30'}`}>
          ...
          <div className="flex flex-wrap items-center gap-4 w-full sm:w-auto mt-3 sm:mt-0">
            ...
            <div className={`flex gap-1 p-1.5 rounded-2xl border transition-colors ${isDarkMode ? 'bg-black/40 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
              <button
                ...
                className={`p-2 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-[#00D4FF]/20 text-[#00D4FF]' : 'text-slate-400 hover:text-slate-200'}`}
              ...
              <button
                ...
                className={`p-2 rounded-xl transition-all ${viewMode === 'table' ? 'bg-[#00D4FF]/20 text-[#00D4FF]' : 'text-slate-400 hover:text-slate-200'}`}
        ...
        {loading && usersList.length === 0 ? (
          ...
          <div className="p-12 text-center text-[#E30A17] font-black tracking-wide drop-shadow-md relative z-10">
            System Error: {errorMsg}
          </div>
        ) : filteredUsers.length === 0 ? (
          <div className={`p-12 text-center font-bold italic relative z-10 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
            No users registered matching these parameters.
          </div>
        ) : viewMode === 'grid' ? (
          /* Modern Card Grid View Grouped by Role */
          <div className="space-y-12 relative z-10 mt-6 animate-in fade-in duration-300">
            {['admin', 'instructor', 'parent', 'student'].map(role => {
              ...
              return (
                <div key={role} className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`w-2.5 h-6 rounded-full bg-gradient-to-b ${roleTheme.bgGrad}`} />
                    <h3 className={`text-sm font-black uppercase tracking-wider flex items-center gap-2 ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>
                      {role === 'admin' ? 'Administrators' : role === 'instructor' ? 'Instructors' : role === 'parent' ? 'Parents' : 'Students'}
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${roleTheme.isStaff ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-cyan-500/10 text-[#00D4FF] border-cyan-500/20'}`}>
                        {usersInRole.length}
                      </span>
                    </h3>
                  </div>
                        ...
                        return (
                          <motion.div
                            ...
                            className={`rounded-3xl border p-6 flex flex-col justify-between transition-all duration-500 relative group shadow-lg ${
                              ...
                              roleTheme.isStaff 
                                ? 'border-blue-500/20 hover:border-blue-500 hover:shadow-[0_10px_30px_rgba(37,99,235,0.12)]' 
                                : 'border-[#00D4FF]/20 hover:border-[#00D4FF] hover:shadow-[0_10px_30px_rgba(0,212,255,0.12)]'
                            }`}
                          >
                            {/* Role Gradient Accent in Top Right */}
                            <div className={`absolute top-0 right-0 w-32 h-32 opacity-[0.03] group-hover:opacity-[0.06] rounded-bl-full pointer-events-none transition-all duration-500 bg-gradient-to-br ${roleTheme.bgGrad}`}></div>
                              ...
                              <div className="relative z-10 flex items-start justify-between gap-3 mb-4">
                                <div className="flex items-center gap-4">
                                  <div className="relative">
                                    <UserAvatar user={u} className={`w-12 h-12 text-sm shadow-md border ${roleTheme.avatarBorder}`} />
                                    {u.role === 'admin' && (
                                      <ShieldCheck className="w-5 h-5 text-[#00D4FF] absolute -bottom-1 -right-1 drop-shadow-md bg-[#0B1120] rounded-full p-0.5 border border-white/10" />
                                    )}
                                  </div>
                                  <div className="flex flex-col min-w-0">
                                    <h4 className={`font-black text-base truncate leading-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{u.name || 'Unknown'}</h4>
                                    <span className={`text-xs truncate font-medium mt-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>{u.email}</span>
                                  </div>
                                </div>
                                
                                {/* Role Badge */}
                                <span className={`px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider shadow-sm text-white ${roleTheme.badgeBg}`}>
                                  {u.role}
                                </span>
                              </div>
                              ...
                              <div className="relative z-10 space-y-4 flex-1 flex flex-col justify-between">
                                <div className={`border p-3.5 rounded-2xl flex flex-col gap-2.5 ${isDarkMode ? 'bg-[#0F172A]/50 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
                                  ...
                                  <div className="flex items-center justify-between text-xs">
                                    <span className={`font-black text-[10px] ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Access State</span>
                                    {u.status === 'pending' ? (
                                      <div className="flex gap-1.5 z-20">
                                        <button onClick={() => updateUserStatus(u.id, 'approved')} className="text-[10px] uppercase tracking-wider font-black bg-[#00D4FF]/20 text-[#00D4FF] hover:bg-[#00D4FF]/30 px-2.5 py-1 rounded-lg border border-[#00D4FF]/30 transition-all shadow-sm">Approve</button>
                                        <button onClick={() => updateUserStatus(u.id, 'rejected')} className="text-[10px] uppercase tracking-wider font-black bg-[#E30A17]/20 text-[#E30A17] hover:bg-[#E30A17]/30 px-2.5 py-1 rounded-lg border border-[#E30A17]/30 transition-all shadow-sm">Reject</button>
                                      </div>
                                    ) : (
                                  ...
                                  {/* Provisioned Date */}
                                  <div className="flex items-center justify-between text-xs">
                                    <span className={`font-black text-[10px] ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Provisioned</span>
                                  ...
                                  <div className="flex items-center justify-between text-xs">
                                    ...
                                    <span className={`inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 rounded-full text-[10px] font-black tracking-wider border ${u.certificates && u.certificates.length > 0 ? 'bg-[#F97316]/10 text-[#F97316] border-[#F97316]/30' : isDarkMode ? 'bg-[#0B1120]/40 text-slate-500 border-slate-700' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                                      {u.certificates ? u.certificates.length : 0}
                                    </span>
                                  </div>
                                </div>

                                {/* Network Connections (If Student) */}
                                {u.role === 'student' && (
                                  <div className={`p-3.5 rounded-2xl border flex flex-col gap-2.5 ${isDarkMode ? 'bg-[#0F172A]/30 border-white/5' : 'bg-slate-50/50 border-slate-100'}`}>
                                    <div className="text-[10px] font-black uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                                      <Network className="w-3.5 h-3.5" /> Network Map
                                    </div>
                                    <div className="flex flex-col gap-2">
                                      ...
                                      <div className="flex items-center justify-between gap-2">
                                        <span className="text-[10px] font-bold text-slate-400">Parent</span>
                                        <CustomDropdown
                                      ...
                                      {user?.role === 'admin' && (
                                        <div className="flex items-center justify-between gap-2">
                                          <span className="text-[10px] font-bold text-slate-400">Sponsor</span>
                                          <CustomDropdown
                            ...
                            {/* Card Actions Footer */}
                            <div className="relative z-10 mt-6 pt-4 border-t border-slate-200 dark:border-white/5 flex flex-col gap-4">
                              {/* Role Change Dropdown */}
                              <div className="flex items-center justify-between gap-2">
                                <span className={`text-[10px] font-black uppercase tracking-wider ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Role Tier</span>
                                {u.id === user?.id ? (
                                  <span className="text-xs font-semibold italic text-slate-400">Self (Admin)</span>
                                ) : (
                                  <CustomDropdown
                                    value={u.role}
                              ...
                              <div className="flex gap-2">
                                <button 
                                  ...
                                  className={`flex-1 py-2.5 rounded-xl border text-[10px] font-black uppercase tracking-wider transition-all hover:scale-[1.02] shadow-sm flex items-center justify-center gap-1.5 ${
                                    isDarkMode ? 'bg-[#F97316]/10 text-[#F97316] border-[#F97316]/20 hover:bg-[#F97316]/20' : 'bg-orange-50 text-orange-600 border-orange-200 hover:bg-orange-100'
                                  }`}
                                >
                                ...
                                <button 
                                  ...
                                  className={`flex-1 py-2.5 rounded-xl border text-[10px] font-black uppercase tracking-wider transition-all hover:scale-[1.02] shadow-sm flex items-center justify-center gap-1.5 ${
                                    isDarkMode ? 'bg-[#E30A17]/10 text-[#E30A17] border-[#E30A17]/20 hover:bg-[#E30A17]/20' : 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100'
                                  }`}
                                >
                                ...
                                <button 
                                  ...
                                  className="flex-1 py-2.5 rounded-xl border text-[10px] font-black uppercase tracking-wider transition-all hover:scale-[1.02] shadow-sm flex items-center justify-center gap-1.5 bg-gradient-to-r from-[#00D4FF]/10 to-[#2563EB]/10 text-[#00D4FF] border-[#00D4FF]/30 hover:border-[#00D4FF]/50"
                                >
          ...
          /* Modern Tabular Table View Grouped by Role */
                ...
                {['admin', 'instructor', 'parent', 'student'].map(role => {
                  ...
                  return (
                    <React.Fragment key={role}>
                      {/* Divider Header Row */}
                      <tr className={`${isDarkMode ? 'bg-[#0F172A]/80' : 'bg-slate-50/80'}`}>
                        <td colSpan="8" className="px-6 py-3 border-y border-slate-200/50 dark:border-white/5">
                          <div className="flex items-center gap-2.5 font-black text-xs uppercase tracking-wider text-slate-400">
                            <span className={`w-2.5 h-2.5 rounded-full bg-gradient-to-r ${roleTheme.bgGrad}`} />
                            {role === 'admin' ? 'Administrators' : role === 'instructor' ? 'Instructors' : role === 'parent' ? 'Parents' : 'Students'} ({usersInRole.length})
                          </div>
                        </td>
                      </tr>
                      {usersInRole.map((u) => (
