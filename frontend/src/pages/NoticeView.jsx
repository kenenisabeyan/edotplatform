...
export default function NoticeView() {
  ...
  return (
    <div className="animate-in fade-in flex flex-col space-y-8 max-w-6xl mx-auto pb-12">
      ...
      {/* Create Notice Form */}
      {showCreateForm && (
        <div className={`backdrop-blur-xl rounded-3xl border shadow-2xl p-6 md:p-8 relative overflow-hidden animate-in fade-in slide-in-from-top-4 ${isDarkMode ? 'bg-[#0B0E14]/80 border-white/10' : 'bg-white/95 border-slate-200'}`}>
          ...
          <div className={`flex justify-between items-center mb-8 border-b pb-4 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
             ...
             <button onClick={() => setShowCreateForm(false)} className={`font-black text-[10px] px-4 py-2 rounded-lg border transition-colors ${isDarkMode ? 'text-slate-200 hover:text-white bg-[#11151F]/5 border-white/10' : 'text-slate-600 hover:text-slate-900 bg-slate-50 border-slate-200'}`}>Cancel</button>
          </div>
          ...
          <form onSubmit={handleCreateNotice} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 space-y-2">
                    ...
                    <div className={`relative border rounded-xl overflow-hidden focus-within:ring-1 transition-all shadow-inner ${isDarkMode ? 'border-white/10 focus-within:border-[#FFD700]/50 focus-within:ring-[#FFD700]/50 bg-[#11151F]' : 'border-slate-300 focus-within:border-indigo-500 focus-within:ring-indigo-500 bg-white'}`}>
                      <input 
                        type="text" 
                        required
                ...
                <div className="space-y-2">
                    ...
                    <CustomDropdown
                      ...
                      className={`w-full [&>button]:py-3.5 [&>button]:font-medium ${isDarkMode ? '[&>button]:bg-[#11151F] [&>button]:border-white/10 [&>button]:text-white' : '[&>button]:bg-white [&>button]:border-slate-300 [&>button]:text-slate-900'}`}
                    />
                </div>
            </div>
            ...
            <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className={`w-full p-4 border rounded-xl outline-none focus:ring-1 font-medium resize-none shadow-inner transition-all text-sm ${isDarkMode ? 'bg-[#11151F] border-white/10 text-white focus:border-[#FFD700]/50 focus:ring-[#FFD700]/50 placeholder:text-slate-300' : 'bg-white border-slate-300 text-slate-900 focus:border-indigo-500 focus:ring-indigo-500 placeholder:text-slate-400'}`}
                ></textarea>
            </div>
      ...
      {notices.length === 0 && !showCreateForm ? (
        <div className={`p-16 text-center rounded-3xl border backdrop-blur-xl shadow-2xl flex flex-col items-center justify-center relative overflow-hidden ${isDarkMode ? 'bg-[#0B0E14]/80 border-white/10' : 'bg-white/95 border-slate-200'}`}>
           ...
           <div className={`w-24 h-24 rounded-full flex items-center justify-center mb-6 border shadow-inner ${isDarkMode ? 'bg-[#11151F] text-[#FFD700]/30 border-white/5' : 'bg-slate-100 text-slate-300 border-slate-200'}`}>
        ...
        <div className="grid grid-cols-1 gap-6">
          {notices.map((notice) => (
            <div key={notice.id} className={`rounded-3xl border backdrop-blur-xl shadow-2xl overflow-hidden p-6 md:p-8 transition-all duration-300 relative group flex flex-col md:flex-row gap-6 ${isDarkMode ? 'bg-[#0B0E14]/80 border-white/10 hover:shadow-[0_0_30px_rgba(255,215,0,0.1)] hover:border-[#FFD700]/30' : 'bg-white/95 border-slate-200 hover:shadow-lg hover:border-slate-300'}`}>
                ...
                <div className={`w-14 h-14 border rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${isDarkMode ? 'bg-[#11151F] border-white/10 text-[#FFD700]' : 'bg-slate-50 border-slate-200 text-indigo-600'}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 gap-3">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <span className={`text-[10px] font-black px-3 py-1.5 rounded-lg border flex items-center gap-1.5 shadow-inner ${isDarkMode ? 'text-slate-300 bg-[#11151F] border-white/5' : 'text-slate-600 bg-slate-50 border-slate-200'}`}>
                        <Clock className="w-3 h-3" /> {new Date(notice.date || notice.createdAt).toLocaleDateString()}
                      </span>
                    </div>
                  </div>
                  <div className={`p-4 border rounded-xl shadow-inner mt-2 ${isDarkMode ? 'bg-[#11151F] border-white/5' : 'bg-slate-50 border-slate-200'}`}>
