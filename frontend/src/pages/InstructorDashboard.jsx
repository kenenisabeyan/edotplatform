                ...
                <div className="h-[250px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={engagementData} margin={{ top: 5, right: 0, bottom: 0, left: -25 }}>
                      ...
                      />
                      <Bar dataKey="students" fill="#F97316" radius={[4, 4, 0, 0]} maxBarSize={40} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {courses.length === 0 ? (
               <div className={`bg-[#1E293B]/50 backdrop-blur-xl p-12 text-center rounded-2xl border-2 border-dashed shadow-sm flex flex-col items-center justify-center ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                 ...
                 <button 
                  onClick={() => navigate('/instructor/builder')} 
                  className="px-8 py-3.5 bg-gradient-to-r from-[#F97316] to-orange-600 text-white font-black   text-xs rounded-xl hover:-translate-y-0.5 transition-transform shadow-[0_0_20px_rgba(255,215,0,0.3)]"
                 >
                   Start Creating
                 </button>
               </div>
                ...
                <div className="grid grid-cols-1 gap-6">
                  {courses.map(c => (
                    <div key={c.id} className={`backdrop-blur-xl rounded-2xl border shadow-2xl overflow-hidden flex flex-col md:flex-row group transition-all hover:border-[#F97316]/30 relative ${isDarkMode ? 'bg-[#1E293B]/90 border-white/10' : 'bg-white/95 border-slate-200'}`}>
                      <div className="absolute top-0 right-0 w-32 h-32 bg-[#F97316] rounded-bl-full opacity-5 pointer-events-none group-hover:scale-110 transition-transform"></div>
                      <div className={`w-full md:w-64 h-48 md:h-auto shrink-0 relative ${isDarkMode ? 'bg-[#1E293B]' : 'bg-slate-100'}`}>
                         <img 
                        ...
                        />
                         <div className={`absolute top-3 left-3 backdrop-blur-sm px-3 py-1.5 rounded-md text-[9px] font-black text-[#F97316] border ${isDarkMode ? 'bg-[#1E293B]/90 border-white/10' : 'bg-white/95 border-slate-200'}`}>
                           {c.category}
                         </div>
                      </div>
                      ...
                      <div className="flex flex-col flex-1 relative z-10">
                        <div className="p-6 md:p-8 flex-1">
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-2">
                            ...
                            <span className={`shrink-0 inline-flex items-center px-3 py-1 rounded-sm text-[9px] font-black   border ${
                              c.status === 'approved' ? 'bg-[#F97316]/20 text-[#F97316] border-[#F97316]/30' : 
                          ...
                          <p className={`text-xs font-black mb-4 flex flex-wrap items-center gap-x-3 gap-y-2 ${isDarkMode ? 'text-slate-200' : 'text-slate-600'}`}>
                            <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 text-[#F97316]" /> {c.duration} hours</span>
                            <span className="flex items-center gap-1.5"><PlayCircle className="w-3.5 h-3.5 text-[#F97316]" /> {c.lessons?.length || 0} lessons</span>
                        ...
                        <div className={`p-4 md:px-8 border-t flex flex-col sm:flex-row justify-between items-center gap-4 ${isDarkMode ? 'border-white/10 bg-[#1E293B]/50' : 'border-slate-200 bg-slate-100'}`}>
                           ...
                           <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                             {(c.status === 'draft' || c.status === 'rejected') && (
                               <>
                                 <button 
                                  ...
                                  className={`flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-6 py-2.5 font-bold text-[10px] rounded-lg border hover:bg-[#F97316] hover:text-white hover:border-[#F97316] transition-colors ${isDarkMode ? 'bg-[#1E293B]/5 text-white border-white/10' : 'bg-slate-50 text-slate-900 border-slate-200'}`}
                                 >
                                   <PlusCircle className="w-4 h-4" /> Add Lesson
                                 </button>
                                 <button 
                                  onClick={() => handleSubmitReview(c.id)} 
                                  className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-gradient-to-r from-[#F97316] to-orange-600 text-white font-bold   text-[10px] rounded-lg hover:shadow-[0_0_15px_rgba(255,215,0,0.3)] transition-all"
                                 >
                             ...
                             {c.status === 'approved' && (
                                 <span className="inline-flex items-center gap-2 text-[#F97316] text-[10px] font-black   bg-[#F97316]/10 px-4 py-2 rounded-lg border border-[#F97316]/20">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-3xl">
            ...
            <div className={`backdrop-blur-xl p-6 md:p-8 rounded-3xl border shadow-2xl relative overflow-hidden ${isDarkMode ? 'bg-[#1E293B]/90 border-white/10' : 'bg-white/95 border-slate-200'}`}>
              ...
              <form onSubmit={handleCreateCourse} className="space-y-6 relative z-10">
                ...
                <div>
                  <label className="block text-[10px] font-black   text-[#F97316] mb-2">Subject Matter Title <span className="text-red-500">*</span></label>
                  <input 
                    type="text" 
                    value={formData.title} 
                    onChange={e => setFormData({...formData, title: e.target.value})} 
                    required 
                    className={`w-full px-6 py-4 bg-[#1E293B] font-bold border rounded-xl focus:ring-2 focus:ring-[#F97316] focus:border-transparent outline-none transition-all placeholder:text-slate-300 ${isDarkMode ? 'text-white border-white/10' : 'text-slate-900 border-slate-200'}`} 
                    placeholder="E.g., Advanced JavaScript Patterns" 
                  />
                </div>
                
                <div>
                  <label className="block text-[10px] font-black   text-[#F97316] mb-2">Detailed Framework <span className="text-red-500">*</span></label>
                  <textarea 
                    value={formData.description} 
                    onChange={e => setFormData({...formData, description: e.target.value})} 
                    required 
                    className={`w-full px-6 py-4 bg-[#1E293B] font-medium border rounded-xl focus:ring-2 focus:ring-[#F97316] focus:border-transparent outline-none transition-all resize-y placeholder:text-slate-300 ${isDarkMode ? 'text-white border-white/10' : 'text-slate-900 border-slate-200'}`} 
                    rows="5" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[10px] font-black   text-[#F97316] mb-2">Primary Domain</label>
                    <CustomDropdown
                  ...
                  <div>
                    <label className="block text-[10px] font-black   text-[#F97316] mb-2">Time to Mastery (Hours) <span className="text-red-500">*</span></label>
                    <input 
                      type="number" 
                      min="1" 
                      ...
                      required 
                      className={`w-full px-6 py-4 bg-[#1E293B] font-bold border rounded-xl focus:ring-2 focus:ring-[#F97316] focus:border-transparent outline-none transition-all ${isDarkMode ? 'text-white border-white/10' : 'text-slate-900 border-slate-200'}`} 
                    />
                  </div>
                </div>

                <div>
                  <label className="block text-[10px] font-black   text-[#F97316] mb-2">Visual Asset Node (URL)</label>
                  <input 
                    type="url" 
                    value={formData.thumbnail} 
                    onChange={e => setFormData({...formData, thumbnail: e.target.value})} 
                    className={`w-full px-6 py-4 bg-[#1E293B] font-medium border rounded-xl focus:ring-2 focus:ring-[#F97316] focus:border-transparent outline-none transition-all placeholder:text-slate-300 ${isDarkMode ? 'text-white border-white/10' : 'text-slate-900 border-slate-200'}`} 
                  ...
                  <p className={`mt-2 text-xs font-bold flex items-center gap-1 ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                    <AlertCircle className="w-4 h-4 text-[#F97316]" /> Will fallback to system default if empty.
                  </p>
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 sm:px-0">
          ...
          <div className={`bg-[#1E293B] border rounded-3xl w-full max-w-xl shadow-2xl relative z-10 animate-in zoom-in-95 duration-200 ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
            ...
            <form onSubmit={handleAddLesson} className="p-8 space-y-6">
              <div>
                <label className="block text-[10px] font-black   text-[#F97316] mb-2">Module Designation <span className="text-red-500">*</span></label>
                <input 
                  type="text" 
                  value={lessonData.title} 
                  onChange={e => setLessonData({...lessonData, title: e.target.value})} 
                  required 
                  className={`w-full px-6 py-4 font-bold border rounded-xl focus:ring-2 focus:ring-[#F97316] outline-none transition-all placeholder:text-slate-300 ${isDarkMode ? 'bg-[#1E293B] text-white border-white/10' : 'bg-white text-slate-900 border-slate-200'}`} 
                  placeholder="E.g., Quantum Algorithm Analysis" 
                />
              </div>
              
              <div>
                <label className="block text-[10px] font-black   text-[#F97316] mb-2">Execution Protocol <span className="text-red-500">*</span></label>
                <textarea 
                  value={lessonData.description} 
                  onChange={e => setLessonData({...lessonData, description: e.target.value})} 
                  required 
                  className={`w-full px-6 py-4 font-medium border rounded-xl focus:ring-2 focus:ring-[#F97316] outline-none transition-all resize-none placeholder:text-slate-300 ${isDarkMode ? 'bg-[#1E293B] text-white border-white/10' : 'bg-white text-slate-900 border-slate-200'}`} 
                  rows="3" 
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                <div className="flex-1">
                  <label className="block text-[10px] font-black   text-[#F97316] mb-2">Media Origin Vector <span className="text-red-500">*</span></label>
                  <input 
                    type="url" 
                ...
                <div className="w-full sm:w-32 shrink-0">
                  <label className="block text-[10px] font-black   text-[#F97316] mb-2">Cycle (m) <span className="text-red-500">*</span></label>
                  <input 
                    type="number" 
                    min="1" 
                    ...
                    required 
                    className={`w-full px-6 py-4 font-black border rounded-xl focus:ring-2 focus:ring-[#F97316] outline-none transition-all text-center ${isDarkMode ? 'bg-[#1E293B] text-white border-white/10' : 'bg-white text-slate-900 border-slate-200'}`} 
                  />
                </div>
              </div>
