...
export default function AgendaWidget({ events, userRole, isAdmin, onDelete, onCreateClick }) {
  ...
  return (
    <>
      <div className={`flex flex-col h-full backdrop-blur-xl border rounded-2xl p-5 shadow-lg relative min-h-[350px] ${isDarkMode ? 'bg-[#0B1120]/40 border-white/5' : 'bg-white/90 border-slate-200'}`}>
        {/* Header */}
            ...
            filteredEvents.map((evt) => {
              ...
              return (
                 <div 
                   ...
                   className={`p-3.5 rounded-xl border transition-all cursor-pointer group flex flex-col gap-2 relative overflow-hidden ${isDarkMode ? 'bg-[#0B1120]/60 border-white/5 hover:border-white/10 hover:bg-white/5/80' : 'bg-slate-50 border-slate-200 hover:border-slate-300 hover:bg-slate-100 shadow-sm'}`}
                 >
      ...
      <PremiumModal isOpen={!!selectedEvent} onClose={() => setSelectedEvent(null)} maxWidth="max-w-lg">
               ...
               <div className={`p-5 border-t flex justify-end ${isDarkMode ? 'border-white/5 bg-[#0B1120]/40' : 'border-slate-200 bg-slate-50/80'}`}>
                 <button 
