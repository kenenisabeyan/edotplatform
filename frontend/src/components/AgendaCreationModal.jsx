...
export default function AgendaCreationModal({ isOpen, onClose, onAgendaCreated, defaultType, defaultTitle, defaultTargetAudiences }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0B0E14]/60 flex items-center justify-center z-[100] p-4 backdrop-blur-md">
      <div className={`backdrop-blur-2xl rounded-3xl w-full max-w-2xl border shadow-[0_0_40px_rgba(0,0,0,0.5)] relative animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col ${isDarkMode ? 'bg-[#11151F]/40 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
        {/* Header */}
                   ...
                   <div>
                     ...
                     <div className="relative">
                        <input 
                          ...
                          className={`w-full pl-10 pr-4 py-3 bg-[#0B0E14]/50 border rounded-xl text-sm focus:border-[#FFD700]/50 focus:ring-1 focus:ring-[#FFD700]/50 font-medium outline-none color-scheme-dark ${isDarkMode ? 'border-white/10 text-white' : 'border-slate-200 text-slate-900'}`} 
                        />
                      ...
                      {categories.map(cat => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${isSelected ? `bg-[#11151F]/5 border-${cat.color.split('-')[1]}-500 shadow-sm` : 'bg-[#0B0E14]/50 hover:border-white/20'} ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}
                          >
                             <div className={`w-8 h-8 rounded-full mb-1 flex items-center justify-center ${isSelected ? cat.color + ' shadow-md' : 'bg-[#11151F]/5 '} ${isDarkMode ? 'text-white text-slate-300' : 'text-slate-900 text-slate-500'}`}>
        ...
        <div className={`p-6 border-t shrink-0 bg-transparent rounded-b-3xl flex justify-end gap-3 glass-card ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
           <button type="button" onClick={onClose} className={`px-6 py-2.5 rounded-xl font-medium hover:text-white hover:bg-[#11151F]/5 transition-colors text-sm ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
              Cancel
           </button>
           <button 
             type="submit" 
