...
export default function ActivityFeed({ isAdmin = false, feedType, limit = 5 }) {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-40">
        <div className={`w-8 h-8 border-4 border-t-[#FFD700] rounded-full animate-spin ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}></div>
      </div>
    );
  }
  ...
  if (activities.length === 0) {
    return (
      <div className={`bg-[#0B0E14]/40 rounded-2xl border p-8 text-center flex flex-col items-center ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
        <div className={`w-12 h-12 border rounded-full flex items-center justify-center mb-3 ${isDarkMode ? 'bg-[#11151F]/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
      ...
      {activities.map((activity, index) => (
        <div key={activity.id || index} className="flex gap-4 items-start relative group">
          {index !== activities.length - 1 && (
            <div className={`absolute left-6 top-10 bottom-[-1rem] w-px -z-10 group-hover:bg-[#FFD700]/20 transition-colors ${isDarkMode ? 'bg-[#11151F]/5' : 'bg-slate-50'}`}></div>
          )}
          ...
          <div className={`flex-1 bg-[#11151F]/80 p-4 rounded-2xl border shadow-lg shadow-black/20 hover:border-white/10 transition-colors backdrop-blur-sm ${isDarkMode ? 'border-white/5' : 'border-slate-100'}`}>
            <div className="flex justify-between items-start gap-2 mb-2">
              <h4 className={`font-bold leading-tight ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{activity.action}</h4>
              <span className={`text-[10px] font-black shrink-0 whitespace-nowrap px-2.5 py-1 rounded-md border ${isDarkMode ? 'text-slate-300 bg-[#11151F]/5 border-white/5' : 'text-slate-500 bg-slate-50 border-slate-100'}`}>
            ...
            {isAdmin && activity.user && (
              <p className={`text-xs mb-2 font-medium ${isDarkMode ? 'text-slate-200' : 'text-slate-600'}`}>
                <span className={`font-bold text-[9px] ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>User:</span> {activity.user.name} <span className="text-[9px] font-black   bg-[#FFD700]/10 border border-[#FFD700]/20 text-[#FFD700] px-1.5 py-0.5 rounded ml-1">{activity.user.role}</span>
              </p>
            )}
            {activity.details && (
              <p className={`text-xs font-medium mt-2 p-3 rounded-xl border shadow-inner ${isDarkMode ? 'text-slate-300 bg-[#0B0E14] border-white/5' : 'text-slate-500 bg-white border-slate-100'}`}>
                {activity.details}
              </p>
            )}
            ...
            <div className="flex items-center justify-between mt-4">
               <div className="flex items-center gap-2">
                 {activity.visibility === 'private' && <span className={`text-[9px] font-black border px-2 py-1 rounded-md shadow-sm ${isDarkMode ? 'text-slate-300 bg-[#11151F]/5 border-white/10' : 'text-slate-500 bg-slate-50 border-slate-200'}`}>Private</span>}
               ...
               {user?.role === 'parent' && activity.visibility === 'insight' && activity.insightFlag === 'concern' && (
                 <div className="flex gap-2">
                   <button onClick={() => navigate('/dashboard/message')} className="text-[10px] font-black   text-[#FFD700] hover:bg-[#FFD700]/20 bg-[#FFD700]/10 border border-[#FFD700]/20 px-3 py-1 rounded-md transition-colors flex items-center gap-1.5 shadow-[0_0_10px_rgba(255,215,0,0.1)]">
