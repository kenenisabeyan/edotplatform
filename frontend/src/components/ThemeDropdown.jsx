...
export default function ThemeDropdown() {
  ...
  return (
    <button
      ...
      className={`p-2 rounded-full transition-colors border shadow-sm ${isDarkMode ? 'hover:bg-white/5 bg-[#0B1120] border-white/10' : 'hover:bg-slate-100 bg-white border-slate-200'}`}
      aria-label="Toggle Theme"
    >
      {isDarkMode ? <Sun className="w-5 h-5 text-[#00D4FF]" /> : <Moon className="w-5 h-5 text-indigo-600" />}
    </button>
  );
}
