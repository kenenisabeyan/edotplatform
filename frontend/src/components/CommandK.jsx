    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="overflow-hidden rounded-3xl bg-[#0B0E14]/95 shadow-2xl shadow-[#FFD700]/10 border border-[#FFD700]/30 backdrop-blur-2xl">
              
              {/* Search Header */}
              <div className={`relative flex items-center px-5 py-4 border-b ${isDarkMode ? 'border-white/10' : 'border-slate-200'}`}>
                <Search className={`w-6 h-6 transition-colors ${isSearching ? 'text-[#FFD700] animate-pulse' : 'text-[#FFD700]'}`} />
                <input
                  ref={inputRef}
                ...
                <button
                  onClick={() => setIsOpen(false)}
                  className={`px-2.5 py-1 text-[10px] font-black rounded-lg border hover:text-white transition-colors ${isDarkMode ? 'text-slate-200 bg-[#11151F]/5 border-white/10' : 'text-slate-600 bg-slate-50 border-slate-200'}`}
                >
                  ESC
                </button>
                )}
                          ...
                          {items.map((item) => {
                            ...
                            return (
                              <li key={`${item.type}-${item.id || item.title}`}>
                                ...
                                >
                                  <div className={`p-2 rounded-xl transition-colors ${isSelected ? 'bg-[#FFD700] text-[#0B0E14]' : 'bg-[#11151F]/5 '} ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
              ...
              <div className="flex items-center justify-between px-6 py-3 border-t border-[#FFD700]/20 bg-black/40 backdrop-blur-md shrink-0">
                <span className={`text-[10px] flex items-center gap-1.5 font-bold ${isDarkMode ? 'text-slate-300' : 'text-slate-500'}`}>
                  Navigate <span className={`px-1.5 py-0.5 rounded border ${isDarkMode ? 'bg-[#11151F]/5 border-white/10 text-slate-300' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>&uarr;</span> <span className={`px-1.5 py-0.5 rounded border ${isDarkMode ? 'bg-[#11151F]/5 border-white/10 text-slate-300' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>&darr;</span>
                </span>
                <span className="text-[10px] text-[#FFD700]/70 flex items-center gap-1.5 font-bold  ">
